...
[![MeleeCombatSystem 시연 영상](https://img.youtube.com/vi/szJrgEgalvU/0.jpg)](https://www.youtube.com/watch?v=szJrgEgalvU)
...
- 적 후방의 AssassinateBoundary 진입 시 주기적으로 암살 조건 판정
...
- 시각과 피격은 즉시 전투 전환, 청각은 게이지 누적 방식으로 점진적 경계 상승 후 임계치 도달 시 전투 전환
...
- 일반 (`BP_MobEnemy`): 근거리 공격, 랜덤 공격 패턴 선택
- 중장형 (`BP_HeavyMobEnemy`): 일반 적과 동일 전투 구조, 별도 스탯/애니메이션 적용
- 보스 (`BP_BossEnemy`): 중거리·근거리 공격 분기, 쿨다운 기반 중거리 공격, 확률 기반 희귀 공격 패턴
- 공통: 체력바 / 청각 게이지 위젯 표시, 전투 진입 시 무기 소켓 전환, 사망 시 무기 물리 드랍

### Dynamic BGM

- 평시 / 전투 / 보스 상황에 따라 BGM 자동 전환
- Combat Enemy List를 기준으로 전투 상태를 추적하고, 리스트가 비워지면 평시 BGM으로 복귀
...
- AIPerception의 감지 결과를 즉시 반응이 아닌 Blackboard Enum 기반 상태 전이 흐름으로 연결
